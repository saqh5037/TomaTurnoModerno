...
export async function assignNextHolding(userId) {
  ...
  if (existingHolding) {
    // Asignación manual del admin: respetar siempre, sin swap por prioridad
    if (existingHolding.forcedAssign) {
      console.log(`[HoldingUtils] Holding ${existingHolding.id} es asignación admin (forcedAssign=true), respetando sin swap`);
      return {
        ...existingHolding,
