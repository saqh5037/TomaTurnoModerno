...
export async function assignNextHolding(userId) {
  ...
  // Orden: Special primero, luego por isDeferred (no diferidos primero), luego por fecha de creación/diferido
  const nextTurn = await prisma.$transaction(async (tx) => {
    // Buscar turno disponible con bloqueo
    const turn = await tx.turnRequest.findFirst({
      where: {
        status: "Pending",
        holdingBy: null, // No está en holding por nadie
      },
      orderBy: [
        { tipoAtencion: "desc" }, // Special (alfabéticamente mayor) primero
        { isDeferred: "asc" }, // No diferidos primero
        { deferredAt: "asc" }, // Para diferidos, por fecha de diferido
        { createdAt: "asc" }, // Por fecha de creación
      ],
    });
