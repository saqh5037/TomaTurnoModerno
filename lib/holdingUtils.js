...
import { CUBICULOS_POR_TIPO, saltaFila, requiresSpecialCubicle } from "./prioridadUtils.js";
...
export async function assignNextHolding(userId) {
  ...
  if (userQueue.length > 0) {
    ...
    if (!currentSaltaFila) {
      ...
      } else if (cubicleType === 'SPECIAL' && !isEspecialTurn(existingHolding.tipoAtencion)) {
  ...
  const nextTurn = await prisma.$transaction(async (tx) => {
    ...
    // FX3 (v2.8.55): Solo MuyEspecial requiere cubículo SPECIAL (cub 6, presos).
    // RiesgoCaida/PrioritarioRiesgo van a GENERAL — antes quedaban varados por bug.
    ...
    // MuyEspecial: cubículo SPECIAL es restricción dura
    // PrioritarioRiesgo: cubículos 1,2 son preferencia (GENERAL)
    let selectedTurnId = null;
    let fallbackTurnId = null;
    for (const turn of turns) {
      const needsSpecial = requiresSpecialCubicle(turn.tipoAtencion);

      // FX3: Restricción por tipo de cubículo
      if (cubicleType === 'GENERAL' && needsSpecial) {
        // Cubículo GENERAL no recibe MuyEspecial (debe ir a cub 6)
        continue;
      }
      if (cubicleType === 'SPECIAL' && !needsSpecial && hasMuyEspecialPending) {
        // Cubículo SPECIAL con MuyEspecial esperando no toma a los demás todavía
        continue;
      }
