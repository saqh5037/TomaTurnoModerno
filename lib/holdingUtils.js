...
export async function assignNextHolding(userId) {
  ...
  if (existingHolding) {
    // Si el holding actual es General, verificar si hay un Special disponible para hacer swap
    if (existingHolding.tipoAtencion !== "Special") {
      const specialAvailable = await prisma.turnRequest.findFirst({
        where: {
          status: "Pending",
          holdingBy: null,
      ...
      if (specialAvailable) {
        // Swap: liberar el General y dejar que se asigne el Special abajo
        console.log(`[HoldingUtils] Swap de prioridad: liberando General ${existingHolding.id} para asignar Special ${specialAvailable.id}`);
        await prisma.turnRequest.update({
          where: { id: existingHolding.id },
          data: { holdingBy: null, holdingAt: null },
        });
        // No retornar — caer al bloque de asignación de abajo
      } else {
      ...
      console.log(`[HoldingUtils] Usuario ${userId} ya tiene turno Special ${existingHolding.id} en holding`);
      return {
        ...existingHolding,
        isSpecial: true
      };
  ...
  const nextTurn = await prisma.$transaction(async (tx) => {
    ...
    return {
      ...updatedTurn,
      isSpecial: updatedTurn.tipoAtencion === "Special"
    };
  });
