...
export async function assignNextHolding(userId) {
  ...
  const cubicleName = userSession?.cubicle?.name || null;
  ...
  const nextTurn = await prisma.$transaction(async (tx) => {
    ...
    for (const turn of turns) {
      ...
      if (preferidos) {
        // Paciente tiene restricción de cubículo
        if (cubicleName && preferidos.map(String).includes(String(cubicleName))) {
          // Flebotomista está en un cubículo compatible — asignar
          selectedTurnId = turn.id;
          break;
        }
        // Flebotomista NO está en cubículo compatible — saltar este paciente
        continue;
      }
    ...
    if (!selectedTurnId) {
      return null; // No hay turnos compatibles con este cubículo
    }
