...
import { CUBICULOS_POR_TIPO, saltaFila, isEspecialTurn } from "./prioridadUtils.js";
...
export async function assignNextHolding(userId) {
  ...
  const nextTurn = await prisma.$transaction(async (tx) => {
    ...
    // FX3: Verificar si hay pacientes especiales en cola
    const hasEspecialPending = turns.some(t => isEspecialTurn(t.tipoAtencion));
    ...
    // FX3: SPECIAL/GENERAL routing por tipo de cubículo
    let selectedTurnId = null;
    let fallbackTurnId = null;
    for (const turn of turns) {
      const turnIsEspecial = isEspecialTurn(turn.tipoAtencion);

      // FX3: Restricción por tipo de cubículo
      if (cubicleType === 'GENERAL' && turnIsEspecial) {
        // Cubículo GENERAL nunca recibe turnos especiales
        continue;
      }
      if (cubicleType === 'SPECIAL' && !turnIsEspecial && hasEspecialPending) {
        // Cubículo SPECIAL con especiales esperando no recibe generales todavía
        continue;
      }
