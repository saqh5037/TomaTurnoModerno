...
/**
 * Determina si un tipo de atención REQUIERE un cubículo de tipo SPECIAL (enum CubicleType).
 *
 * Tabla de routing por enum de cubículo (v2.8.56 — alineado al diagrama del docx):
 *   - MuyEspecial      → solo SPECIAL (cub 6, presos)
 *   - Prioritario      → solo SPECIAL (vulnerable sin riesgo de caída)
 *   - PrioritarioRiesgo → cubs 1, 2 GENERAL (preferencia dura por accesibilidad)
 *   - RiesgoCaida      → cubs 1, 2 GENERAL (preferencia, FIFO puro)
 *   - General          → cualquier cubículo (SPECIAL solo si no hay vulnerables del tipo SPECIAL esperando)
 */
export function requiresSpecialCubicle(tipoAtencion) {
  return tipoAtencion === 'MuyEspecial' || tipoAtencion === 'Prioritario';
}

/**
 * @deprecated Usar requiresSpecialCubicle. Mantenido como alias durante migración.
 * Antes incluía a TODOS los tipos != General, lo cual rompía el routing de
 * RiesgoCaida/PrioritarioRiesgo en cubículos GENERAL (cubs 1, 2).
 */
export const isEspecialTurn = requiresSpecialCubicle;

/**
 * Extracts the trailing number from a cubicle name string.
 * Handles names like "1", "Cubículo 3", "Cubículo 12", etc.
 * @param {string|null|undefined} name - Cubicle name
 * @returns {number|null} Numeric portion or null if not found
 */
export function cubicleNumberFromName(name) {
...
/**
 * Returns true if the cubicle name matches any of the preferred cubicle numbers.
 * @param {string|null|undefined} name - Cubicle name
 * @param {number[]} preferidos - Array of preferred cubicle numbers
 * @returns {boolean}
 */
export function cubicleMatchesPreferred(name, preferidos) {
  const num = cubicleNumberFromName(name);
  return num != null && preferidos.includes(num);
}
