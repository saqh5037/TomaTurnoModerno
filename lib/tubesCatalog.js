...
export function enrichTubesDetails(tubesDetails) {
  ...
  return tubesDetails.map(tube => {
    ...
    // KAB-7378: Si no se encuentra en catálogo, usar datos de LABSIS directamente
    if (!catalogTube) {
      // Si tiene containerType de LABSIS, usar esos datos
      if (tube.containerType || tube.containerCode) {
        return {
          type: tube.type || tube.containerCode?.toLowerCase() || 'desconocido',
          code: tube.containerCode || tube.type?.toUpperCase() || 'DESC',
          name: tube.containerType || `Tubo ${tube.containerCode || tube.type || 'Desconocido'}`,
          color: 'Gris',
          colorHex: '#808080',  // Gris para tubos no mapeados
          quantity: tube.quantity,
          sampleType: tube.sampleType || null,
          containerCode: tube.containerCode || null,
          containerType: tube.containerType || null
        };
      }
      return null;
    }
    ...
    return {
      type: tube.type,
      code: tube.containerCode || catalogTube.code,  // KAB-7378: Preferir código LABSIS
      // KAB-7378: Preferir containerType de LABSIS si está disponible
      name: tube.containerType || catalogTube.name,
      color: catalogTube.color,
      colorHex: catalogTube.colorHex,
      quantity: tube.quantity,
      sampleType: tube.sampleType || null,  // Preservar sampleType de LABSIS
      // KAB-7378: Preservar campos originales de LABSIS
      containerCode: tube.containerCode || null,
      containerType: tube.containerType || null
    };
