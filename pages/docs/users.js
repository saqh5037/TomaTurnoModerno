...
const UsersDocumentation = ({ moduleData }) => {
        ...
        <Grid templateColumns={{ base: '1fr', lg: '3fr 1fr' }} gap={8}>
          <VStack align="stretch" spacing={8}>
            ...
            {content.sections && content.sections.length > 0 && (
              <GlassCard p={8} animation={`${fadeInUp} 1s ease-out`}>
...
export async function getStaticProps() {
  ...
  const moduleData = fullDocumentation.find(m => m.moduleId === 'users');
...
export default function ProtectedUsersDocumentation({ moduleData }) {
  return (
    <ProtectedRoute>
      <UsersDocumentation moduleData={moduleData} />
    </ProtectedRoute>
  );
}
