...
import {
  ...
  FaClock, FaBook, FaImage, FaChevronRight, FaHome, FaInfoCircle, FaChartBar
} from 'react-icons/fa';
import ReactMarkdown from 'react-markdown';
import ProtectedRoute from '../../components/ProtectedRoute';
import { GlassCard, ModernHeader, fadeInUp, slideInFromLeft } from '../../components/theme/ModernTheme';

const EstadisticasDocumentation = ({ moduleData }) => {
        ...
        >
          ...
          <HStack spacing={6} align="start" mb={6}>
            ...
            <Circle
              ...
              flexShrink={0}
            >
              <Icon as={FaChartBar} fontSize="36px" />
            </Circle>
...
export async function getStaticProps() {
  ...
  const moduleData = fullDocumentation.find(m => m.moduleId === 'estadisticas');
...
export default function ProtectedEstadisticasDocumentation({ moduleData }) {
  return (
    <ProtectedRoute>
      <EstadisticasDocumentation moduleData={moduleData} />
    </ProtectedRoute>
  );
}
